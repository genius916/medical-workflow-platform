import type { Plugin } from 'vite'

// 知识库内联（由 1-中西医内科诊断公式+辨证秒杀词(OCR).pdf 用 pymupdf 提取）
const examKnowledge = "懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n中西医结合内科学\n一、呼吸系统疾病\n诊断公式\n发热、鼻塞、咳嗽、流涕+淋巴细胞比例1(肺部无啰音、X线无阴影)\n急性上呼\n风寒束表\n恶寒重，发热轻，脉浮或浮紧\n荆防败毒散\n吸道感染\n辨证论治\n风热犯表\n流浊涕，痰黄黏稠，脉浮数\n银翘散或葱豉桔梗汤\n暑湿伤表\n肢体酸重，口中黏腻，胸脘痞闷，脉濡数\n新加香薷饮\n诊断公式\n急性咳嗽、咳痰+两肺散在干、湿啰音+胸部X线片、血象\n风寒袭肺\n痰稀色白，恶寒，发热，苔薄白，脉浮紧\n三拗汤合止嗽散\n急性气管-\n支气管炎\n风热犯肺\n咳痰稠黄，流黄涕，头痛口渴，苔薄黄，脉浮数\n桑菊饮\n辨证论治\n燥热伤肺\n干咳无痰，鼻燥咽干，苔薄黄而干，脉浮数\n桑杏汤\n凉燥伤肺\n干咳无痰，咽干鼻燥，苔薄白而干，脉浮紧\n杏苏散地人医考\n诊断公式\n咳、痰或伴喘，每年发病持续3个月，并连续2年或以上\n实证-风寒犯肺\n痰白量多，恶寒或发热，脉浮紧\n三拗汤合止嗽散\n实证-风热犯肺\n鼻流黄涕，身热汗出，脉浮\n桑菊饮\n实证-痰湿蕴肺\n痰多色白而黏，苔白腻，脉滑\n二陈汤合三子养亲汤\n慢性支气\n管炎\n实证-痰热郁肺\n痰多色黄黏稠，苔黄腻，脉滑数\n清金化痰汤\n辨证论治\n实证-寒饮伏肺\n喘逆不得卧，咳吐清稀白沫痰，苔白滑，脉弦紧\n小青龙汤\n虚证-肺气虚\n气短，反复易感，自汗畏风，脉细弱\n玉屏风散\n虚证-肺脾气虚\n气短乏力，食后腹胀，便溏\n补肺汤\n虚证-肺肾气阴两虚气促，潮热盗汗，手足心热，腰酸耳鸣，脉细数\n沙参麦冬汤合六味地黄丸\n诊断公式\n老年人+咳、痰、喘、桶状胸+过清音+肺功能异常(FEV,/FVC<70%)\n外寒内饮\n咳逆喘息，恶寒发热，背冷无汗，苔白滑脉弦紧\n小青龙汤\n痰热郁肺\n痰黄，舌红，苔黄腻，脉滑数\n越婢加半夏汤或桑白皮汤\n慢性阻塞\n性肺疾病\n痰浊壅肺\n痰多色白黏腻，苔薄腻或浊腻，脉滑\n三子养亲汤合二陈汤\n辨证论治\n肺脾气虚\n咳喘气短，腹胀，食少便溏，脉细弱\n补肺汤合四君子汤\n肺肾气虚\n呼吸浅短难续，声低气怯，腰膝酸软\n平喘固本汤合补肺汤\n阳虚水泛\n喘咳不能平卧，面浮，肢肿，脉沉细\n真武汤合五苓散\n诊断公式\n老年人+慢性呼吸系统病史+肺动脉高压(P?>A?)+右心室肥大或右心衰体征\n急性期-痰浊壅肺\n痰多色白黏腻，苔浊腻，脉滑\n苏子降气汤\n急性期-痰热郁肺\n痰黄，溲黄便干，苔黄腻，脉滑数\n越婢加半夏汤\n慢性肺源\n性心脏病\n急性期-痰蒙神窍\n谵语，昏迷，肢体暇动，抽搐，咳痰\n涤痰汤，另服安宫牛黄丸或至宝丹\n辨证论治\n急性期-阳虚水泛\n面浮，身肿，心悸，咳喘，苔白滑\n真武汤合五苓散\n缓解期-肺肾气虚\n呼吸浅短难续，声低气怯，脉沉细微无力补肺汤\n缓解期-气虚血瘀\n喘咳无力，面色晦暗，脉细涩无力\n生脉散合血府逐瘀汤\n诊断公式\n青少年+过敏史+发作性喘憋+满肺哮鸣音+支气管舒张试验、支气管激发试验阳性\n发作期-寒哮\n天冷或受寒易发，形寒畏冷，脉弦紧\n射干麻黄汤\n支气管哮\n发作期-热哮\n咳痰色黄；舌质红苔黄腻，脉滑数\n定喘汤或越婢加半夏汤\n喘\n辨证论治\n发作期-寒包热哮\n痰黄白相兼，烦躁，发热，恶寒，无汗，身痛\n小青龙加石膏汤或厚朴麻\n黄汤\n发作期-风痰哮\n声如拽锯，鸣声如吹哨笛，倏忽来去，发痒\n三子养亲汤\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n缓解期-肺虚\n气短，语声低微，自汗畏风，脉细弱或虚大\n玉屏风散\n缓解期-脾虚\n倦怠无力，食少便溏，纳呆，脉细弱\n六君子汤\n缓解期-肾虚\n呼多吸少，动则为甚，腰酸腿软\n金匮肾气丸或七味都气丸\n诊断公式\n1.肺炎链球菌肺炎=青壮年+受凉+高热、胸痛、咳铁锈色样痰+肺实变体征、听诊湿啰音+X线\n2.肺炎支原体肺炎=持久的阵发性刺激性呛咳、无痰+胸部X线(多种形态浸润影，节段性分布)\n邪犯肺卫\n发热重，恶寒轻，头痛鼻塞，脉浮数\n三拗汤或桑菊饮\n肺炎\n痰热壅肺\n咳痰黄稠，高热不退，脉洪数或滑数\n麻杏石甘汤合千金苇茎汤\n辨证论治\n热陷心包\n神昏谵语，痰鸣肢厥，高热不退，四肢厥冷\n清营汤合莒蒲郁金汤\n阴竭阳脱\n大汗肢冷，颜面苍白，四肢厥冷，脉微欲绝\n生脉散合四逆汤\n正虚邪恋\n咳嗽声低，手足心热，自汗，脉细数\n竹叶石膏汤\n诊断公式\n>40岁+吸烟史+持续刺激性干咳、咳声呈高音调金属音、痰中带血+X线占位性病变\n气滞血瘀\n胸胁刺痛，面青唇暗，舌质暗紫脉涩\n血府逐瘀汤\n原发性支\n气管肺\n痰湿蕴肺\n痰多，纳差便溏，苔厚腻，脉弦滑\n二陈汤合瓜蒌薤白半夏汤\n辨证论治\n阴虚毒热\n手足心热，邪热炽盛，脉细数或数大\n沙参麦冬汤合五味消毒饮\n气阴两虚\n咳嗽无力，神疲，手足心热，脉细数无力\n生脉散合沙参麦冬汤\n诊断公式\n慢性呼吸系统病史+呼吸困难、紫绀+血气分析(PaD?60mmHg)\n痰浊阻肺\n喉中痰鸣，胸中窒闷，苔白腻，脉滑数\n二陈汤合三子养亲汤\n呼吸衰竭\n肺肾气虚\n呼吸短浅难续，形寒，脉沉细无力\n补肺汤合参蚧散\n(助理不\n脾肾阳虚\n腹部胀满，浮肿，肢冷尿少，苔白滑\n真武汤合五苓散\n考)\n辨证论治\n痰蒙神窍\n痰鸣，谵语，烦躁，嗜睡，抽搐、昏迷\n涤痰汤送服安宫牛黄丸、至\n宝丹\n阳微欲脱\n面色苍白，冷汗淋漓，四肢厥冷，脉微欲绝\n独参汤灌服，同时用参附注\n射液静脉滴注\n二、循环系统疾病\n诊断公式\n1.左心衰=高血压+呼吸困难(劳力性、夜间阵发、端坐呼吸)+肺底部湿啰音+左心扩大；\n2.右心衰=肿(踝部、四肢凹陷性)+胸腔积液+消化道症状+肝大、肝颈静脉回流征\n慢性心力\n气虚血瘀\n气短，神疲乏力，舌质紫暗，脉虚涩\n保元汤合血府逐瘀汤\n衰竭\n气阴两虚\n气短乏力，五心烦热，脉细数\n辨证论治\n生脉饮合血府逐瘀汤\n阳虚水泛\n腰膝酸软，形寒肢冷，肢体浮肿，脉沉弱或迟\n真武汤合葶苈大枣泻肺汤\n痰饮阻肺\n痰多，胸闷脘痞，苔厚腻，脉弦滑而数\n芩桂术甘汤合丹参饮\n诊断公式\n突发呼吸困难、端坐呼吸+咳嗽、咳大量粉红色泡沫样痰+双肺湿啰音+BNP/NT-proBNP升高\n急性心力\n心肺气虚\n心悸，气短，动则加剧，脉沉细或虚数\n养心汤合补肺汤\n衰竭\n辨证论治\n心脾阳虚\n心悸，浮肿，食少纳呆，形寒肢冷，大便溏泄\n真武汤。喘促明显加参蛤散\n心阳欲脱\n面色苍白，四肢厥冷，脉微细\n独参汤或四味回阳饮\n1.期前收缩\n(1)房性：①提早出现的P’波，形态与窦性P波不同；②RP’>0.12秒；③代偿间歇不完全\n(2)房室交界性：提前出现的QRS波，形态正常\n(3)室性：①QRS波群提早出现，宽大、畸形；②T波亦宽大，其方向与QRS主波方向相反；③代偿\n快速性心\n律失常\n诊断公式\n间歇完全\n2.窦性心动过速：P波在1、11、aVF导联直立，aVR倒置。窦性P波频率>100次/分\n3.阵发性室上性心动过速：心率多在160～220次/分，突发突止\n4.室性心动过速：连续3次或以上的室早\n5.房颤：(1)P波消失，代之以大小、形态、间距不一的f波，频率为350～600次/分。(2)RR间\n期绝对不齐地人医考\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyiyikao123\n6.房扑：(1)P波消失，代之以连续性锯齿样f波(各波大小、形态相同，频率规则，为250～350\n次/分);(2)QRS波群及T波均呈正常形态\n心虚胆怯\n善惊易恐，坐卧不安，恶闻声响\n安神定志丸\n心血不足\n食少纳呆，眩晕乏力，面色无华，脉细弱\n归脾汤\n阴虚火旺\n手足心热，舌红，苔少，脉细数\n天王补心丹\n辨证论治\n气阴两虚\n短气，乏力，少气懒言，五心烦热\n生脉散\n痰火扰心\n胸闷，口干口苦，舌苔黄腻，脉弦滑\n黄连温胆汤\n心脉瘀阻\n唇甲青紫，舌质紫暗有瘀斑，脉涩\n桃仁红花煎\n心阳不振\n气短，面色苍白，形寒肢冷，脉虚弱\n参附汤合桂枝甘草龙骨牡蛎汤\n1.窦性心动过缓：窦性心律，心率小于60次/分\n2.房室传导阻滞：\n(1) I度：①窦性P波，每个P波后都有相应的QRS波群；②PR 间期延长至0.20秒以上\n诊断公式\n(2)Ⅱ度：1)l型：PR间期逐渐延长；RR间隔相应地逐渐缩短，直到P波后无QRS波群出现，如\n此周而复始\n2)Ⅱ型：PR间期固定(正常或延长);P波突然不能下传而QRS波脱漏\n缓慢性心\n(3)Ⅲ度：①P波与QRS波群无固定关系；②心房速率快于心室率\n律失常\n3.病态窦房结综合征：(1)心率<50次/分。(2)心动过缓与心动过速交替出现(慢一快综合征)\n心阳不振\n气短，面色苍白，形寒肢冷，脉虚弱\n参附汤合桂枝甘草龙骨牡蛎汤\n心肾阳虚\n气短，形寒肢冷，腰膝酸软，脉沉迟\n参附汤合真武汤\n辨证论治\n气阴两虚\n气短乏力，自汗盗汗，五心烦热\n炙甘草汤\n痰浊阻滞\n痰多，腹胀恶心，苔白腻，脉弦滑\n涤痰汤\n心脉瘀阻\n舌质暗或瘀点、瘀斑，脉结代\n桃仁红花煎\n诊断公式\n冠心病病史+意识突然丧失+无呼吸，或仅是喘息+大动脉搏动消失\n心脏性猝\n气阴两脱\n气短，少苔，脉虚数或微\n生脉散\n死\n辨证论治\n痰蒙神窍\n神志恍惚，喉间痰鸣，苔厚腻\n莒蒲郁金汤\n元阳暴脱\n面色苍白，四肢厥冷，脉微细欲绝\n独参汤或四味回阳饮\n诊断公式\n非同日3次测量血压收缩压均≥140mmHg和(或)舒张压≥90mmHg\n肝阳上亢\n口干口苦，面红目赤，烦躁易怒，脉弦\n天麻钩藤饮\n原发性高\n痰湿内盛\n头重如裹，肢体沉重，舌胖苔腻脉濡滑\n半夏白术天麻汤\n血压\n辨证论治\n瘀血阻窍\n头痛固定不移，舌紫，脉弦细涩\n通窍活血汤\n肝肾阴虚\n目涩，五心烦热，盗汗，腰膝酸软\n杞菊地黄丸\n肾阳虚衰\n形寒肢冷，腰膝酸软，夜尿频多，脉沉弱\n济生肾气丸\n诊断公式\n激动、饱食、运动+胸骨体后疼痛(向左上肢放射)+持续3～5min+舌下含服硝酸甘油缓解+心电图\nST段下移≥0.1mV\n心血瘀阻\n胸痛如刺，痛有定处，脉弦涩\n血府逐瘀汤\n冠状动脉\n痰浊内阻\n胸闷痛如窒，痰多，肢体沉重，脉滑\n瓜萎薤白半夏汤合涤痰汤\n粥样硬化\n阴寒凝滞\n感寒痛甚，形寒，冷汗自出，脉沉紧\n枳实薤白桂枝汤合当归四逆汤\n性心脏病\n一心绞痛\n辨证论治\n气虚血瘀\n乏力，气短懒言，舌质淡暗，脉结、代\n补阳还五汤\n气阴两虚\n气短，心烦，或手足心热，舌红少津\n生脉散合炙甘草汤\n心肾阴虚\n心悸盗汗，腰膝酸软，脉沉细数\n左归丸 人医考\n心肾阳虚\n心悸，畏寒，肢冷，腰酸无力，脉沉细\n参附汤合右归丸\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyiyikao123\n诊断公式\n中老年人+胸痛>30分钟+服用硝酸甘油不缓解+ST段弓背抬高，T波倒置，病理性Q波+肌红蛋白，\n心肌酶升高\n气滞血瘀\n烦躁易怒，唇甲青暗，脉沉弦涩\n血府逐瘀汤\n冠状动脉\n寒凝心脉\n胸痛彻背，形寒畏冷，四肢不温，脉沉紧\n当归四逆汤合苏合香丸\n粥样硬化\n性心脏病\n痰瘀互结\n胸痛如割如刺，痰多，舌苔浊腻，脉滑\n瓜萎薤白半夏汤合桃红四\n物汤\n一急性心\n辨证论治\n肌梗死\n气虚血瘀\n神疲乏力，舌质暗淡，脉细弱无力\n补阳还五汤\n气阴两虚\n气短乏力，心烦，自汗盗汗，脉细数\n生脉散合左归饮\n阳虚水泛\n喘促，畏寒肢冷，腰部浮肿，苔滑，脉沉细\n真武汤合葶苈大枣泻肺汤\n心阳欲脱\n大汗淋漓，面色苍白，脉微欲绝\n参附龙牡汤\n1.二尖瓣狭窄：心尖部舒张期杂音+“梨形心”\n诊断公式\n2.二尖瓣关闭不全：心尖部全收缩期杂音\n3.主动脉瓣狭窄：呼吸困难、心绞痛和晕厥+胸骨右缘第2肋间收缩期杂音\n4.主动脉瓣关闭不全：周围血管征阳性+胸骨左缘第3、4肋间可听到舒张期杂音\n心脏瓣膜\n心肺瘀阻\n面色瘀暗、唇紫，舌质瘀暗或有瘀点\n血府逐瘀汤\n病\n气血亏虚\n心悸气短，面色无华，舌淡，脉细弱\n归脾汤\n辨证论治\n气阴两虚\n心悸气短，盗汗，舌红，脉细数\n炙甘草汤\n气虚血瘀\n心悸气短，口唇青紫，舌有紫斑、瘀点\n独参汤合桃仁红花煎\n心肾阳虚\n浮肿，形寒肢冷，脉沉细无力或结代\n参附汤合五苓散\n诊断公式\n呼吸道/消化道感染史+青年人+发热、呼吸困难+心律失常、肌钙蛋白1\n热毒侵心\n发热微恶寒，鼻塞流涕，脉浮数\n银翘散\n病毒性心\n湿毒犯心\n恶心欲呕，腹胀，大便稀溏，脉濡数\n葛根芩连汤合甘露消毒丹\n肌炎\n辨证论治\n心阴虚损\n心烦，低热盗汗，舌红，少苔，脉细数\n天王补心丹\n气阴两虚\n气短乏力，自汗盗汗，脉细数无力\n炙甘草汤合生脉散\n阴阳两虚\n气短，肢冷畏寒，浮肿，脉沉细无力\n参附养荣汤\n诊断公式\n心脏扩大、心律失常、心力衰竭+超声心动图全心扩大、左室射血分数降低\n扩张型心\n邪毒犯心\n身热微恶寒，咽痛身痛，脉浮数\n银翘散\n肌病(助理\n气虚血瘀\n气短，神疲乏力，舌有瘀点\n圣愈汤合桃红四物汤\n不考)\n辨证论治\n气阴两虚\n心悸气短，盗汗，脉细数无力\n炙甘草汤合天王补心丹\n阳虚水泛\n形寒肢冷，下肢水肿，苔白滑，脉沉细\n真武汤\n心阳虚脱\n大汗淋漓，四肢厥冷，脉细微欲绝\n四逆汤合参附龙牡汤\n三、消化系统疾病\n诊断公式\n非甾体抗炎药/应激+腹痛、腹胀+内镜检查\n寒邪客胃\n遇冷痛剧，得热痛减，喜热饮食，脉弦紧迟\n香苏散合良附丸\n急性胃炎\n湿热中阻\n胃痛灼热，头身重着，苔厚腻，脉弦滑\n清中汤\n(助理不\n饮食伤胃\n饱胀拒按，嗳腐酸臭，苔厚腻，脉弦滑\n保和丸\n考)\n辨证论治\n肝气犯胃\n胀痛，痛窜胁背，气怒痛重，脉弦\n柴胡疏肝散\n胃络瘀阻\n胃脘痛如针刺，痛有定处，拒按，脉弦涩\n失笑散合丹参饮\n脾胃虚寒\n胃脘隐痛，喜暖喜按，倦怠无力，脉细弦紧\n黄芪建中汤\n胃阴不足\n胃热隐痛，五心烦热，舌裂纹少津，脉细数\n一贯煎合芍药甘草汤\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyivikao123\n诊断公式\nHp 感染+消化不良症状+胃镜确诊\n肝胃不和\n痛窜两胁，情志不舒加重，嗳气频频，脉弦\n柴胡疏肝散\n慢性胃炎\n脾胃虚弱\n隐痛，喜温喜按，纳呆便溏，神疲乏力，脉沉细\n四君子汤\n辨证论治\n脾胃湿热\n胃脘灼热嘈杂，腹脘痞闷，舌红，苔黄腻，脉滑\n三仁汤\n胃阴不足\n隐痛嘈杂，五心烦热，舌红少津，脉细\n益胃汤\n胃络瘀阻\n痛如针刺，痛有定处，拒按，脉弦涩\n失笑散合丹参饮\n诊断公式\n1.胃溃疡=上腹周期性疼痛+餐后痛+胃镜/X线见龛影\n2.十二指肠溃疡=上腹周期性疼痛+饥饿痛或午夜痛+胃镜/X线见龛影\n肝胃不和\n胀痛，痛引两胁，情志不遂而诱发，嗳气，脉弦\n柴胡疏肝散合五磨饮子\n消化性溃\n脾胃虚寒\n隐痛，喜温喜按，畏寒肢冷，泛吐清水，脉迟缓\n黄芪建中汤\n疡\n辨证论治\n胃阴不足\n似饥而不欲食，手足心热，舌红少津，脉细数\n益胃汤\n肝胃郁热\n灼热，胸胁胀满，口苦口干，烦躁易怒，脉弦数\n化肝煎合左金丸\n瘀血停胃\n胃痛如刺，痛处固定，舌质紫暗有瘀斑，脉涩\n失笑散合丹参饮\n诊断公式\n上消化道疾病/门脉高压病史+呕血、黑便、失血性周围循环衰竭\n胃中积热\n吐血鲜红，喜冷饮，舌红苔黄，脉滑数\n泻心汤合十灰散\n上消化道\n出血\n肝火犯胃\n口苦目赤，胸胁胀痛，心烦易怒，脉弦数\n龙胆泻肝汤\n辨证论治\n脾不统血\n大便稀溏，头晕心悸，脉细弱\n归脾汤\n气随血脱\n面色苍白，大汗淋漓，四肢厥冷，脉微细\n独参汤或四味回阳饮\n诊断公式\n中老年人+上腹不规律疼痛、消瘦+上腹部扪及肿块+粪便隐血持续阳性+胃镜结合黏膜活检\n痰气交阻\n胃脘满闷作胀，呕吐痰涎，苔白腻，脉弦滑\n启膈散\n肝胃不和\n窜及两胁，嗳气频繁，脉弦\n柴胡疏肝散合旋覆代赭汤\n脾胃虚寒\n喜按喜暖，大便溏薄，脉沉细\n胃癌\n理中汤合四君子汤\n辨证论治\n胃热伤阴\n嘈杂灼热，五心烦热，舌质红绛，脉细数\n玉女煎\n瘀毒内阻\n痛处固定，肌肤甲错，眼眶暗黑，脉弦涩\n膈下逐瘀汤\n痰湿阻胃\n呕吐痰涎，苔白厚腻，脉滑\n开郁二陈汤\n气血两虚\n乏力，面色无华，少气懒言，自汗，脉沉细无力\n八珍汤\n诊断公式\n慢性或反复发作性腹痛(疼痛→便意→便后缓解)+腹泻、黏液脓血便+粪检无病原体+结肠镜检查\n湿热内蕴\n里急后重，肛门灼热；舌红苔黄腻，脉滑数\n白头翁汤\n脾胃虚弱\n腹胀，肢体倦怠，神疲懒言；舌质淡胖\n参苓白术散\n溃疡性结\n肠炎\n脾肾阳虚\n腰酸膝软，形寒肢冷，神疲懒言；脉尺弱\n理中汤合四神丸\n辨证论治\n肝郁脾虚\n情绪紧张或抑郁恼怒等诱因，嗳气，脉弦细\n痛泻要方\n阴血亏虚\n午后发热，盗汗，五心烦热；舌红少苔，脉细数\n驻车丸\n气滞血瘀\n胸胁胀痛，面色晦暗，肌肤甲错，脉弦涩\n膈下逐瘀汤\n诊断公式\n中年人+乙肝病史+蜘蛛痣、肝掌+脾大+移动性浊音阳性\n气滞湿阻\n胁下胀痛，得嗳气稍减，苔薄白腻，脉弦\n柴胡疏肝散合胃苓汤\n寒湿困脾\n按之如囊裹水，水肿，怯寒懒动，苔白滑，脉缓\n实脾饮\n肝硬化\n湿热蕴脾\n烦热口苦，渴不欲饮，大便溏滞不爽，脉弦滑数\n辨证论治\n中满分消丸合茵陈蒿汤\n肝脾血瘀\n胁腹刺痛，舌质紫暗，脉细涩\n调营饮\n脾肾阳虚\n朝宽暮急，神疲怯寒，脘闷纳呆，脉沉迟无力\n附子理中汤合五苓散\n肝肾阴虚\n口干舌燥，舌红绛少津，少苔或无苔，脉弦细数\n一贯煎合膈下逐瘀汤\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyiyikao123\n诊断公式\n肝炎病史+右上腹痛加重+体重下降+AFP↑+B超占位\n原发性肝\n气滞血瘀\n两胁胀痛，嗳气，舌质红有瘀斑，脉弦或涩\n逍遥散合桃红四物汤\n癌\n辨证论治\n湿热瘀毒\n目肤黄染，肌肤甲错，高热烦渴，苔黄腻脉弦数\n茵陈蒿汤合鳖甲煎丸\n肝肾阴虚\n潮热盗汗，头晕耳鸣，腰膝酸软，脉细数\n滋水清肝饮合鳖甲煎丸\n医\n扫码关注\n扫码进入小程序\n如有任何问题\n四、泌尿系统疾病\n了解更多考试资讯\n获取更多\n扫码添加老师咨询\n诊断公式\n慢性血尿+蛋白尿+水肿+血压升高\n本证-脾肾气虚\n腰脊酸痛，神疲乏力，纳呆脘胀，便溏，脉细\n异功散\n本证-肺肾气虚\n乏力，少语懒言，自汗，易感冒，腰脊酸痛\n玉屏风散合金匮肾气丸\n本证-脾肾阳虚\n畏寒肢冷，腰脊冷痛，纳少便溏，遗精，阳痿\n慢性肾小\n附子理中丸或济生肾气丸\n球肾炎\n本证-肝肾阴虚\n目睛干涩，头晕耳鸣，五心烦热，腰膝酸痛\n杞菊地黄丸\n辨证论治\n本证-气阴两虚\n少气乏力，午后低热，舌红少苔，脉细\n参芪地黄汤\n标证-水湿\n颜面或肢体浮肿，苔白腻，脉缓或沉缓\n五苓散合五皮饮\n标证-湿热\n口干不欲饮，胸脘痞闷，舌红苔黄腻，脉滑数\n三仁汤\n标证-血瘀\n面色黧黑，腰刺痛，肌肤甲错，脉细涩\n血府逐瘀汤\n诊断公式\n大量蛋白尿(≥3.5g/d)+低蛋白血症(血浆白蛋白≤30g/L)+明显水肿+高脂血症\n风水相搏\n皮肤凹陷易恢复，发热，咽痛咳嗽，脉浮\n越婢加术汤\n肾病综合\n热毒浸淫\n身发痈疡，恶风发热，舌红苔薄黄，脉浮数\n麻黄连翘赤小豆汤合五味消毒饮\n征(助理不\n水湿浸渍\n按之没指，身重困倦，纳呆，苔白腻，脉濡缓\n五皮饮合胃苓汤\n考)\n辨证论治\n湿热内蕴\n胸闷烦热，舌红苔黄腻，脉濡数\n疏凿饮子\n脾虚湿困\n按之凹陷不易恢复，神疲乏力，大便溏，脉沉弱\n实脾饮\n肾虚水泛\n腰部冷痛酸重，形寒神疲，脉沉迟无力\n济生肾气丸合真武汤\n1.膀胱炎=中青年女性+膀胱刺激征(尿频、尿急、尿痛)+尿细菌培养阳性\n诊断公式\n2.急性肾盂肾炎=育龄妇女+腰痛、发热、膀胱刺激征+肾区叩击痛、肋腰点压痛\n3.慢性肾孟肾炎=急性肾孟肾炎病史+膀胱刺激征反复发作+多次尿细菌培养阳性+肾盂肾盏变形/肾小\n管功能损害\n尿路感染\n膀胱湿热\n灼热刺痛，色黄赤，舌红苔薄黄腻，脉滑数\n八正散\n肝胆郁热\n烦躁易怒，口苦口黏，胸胁苦满，脉弦细\n龙胆泻肝汤\n辨证论治\n脾肾亏虚，湿热屡犯神疲乏力，少气懒言，腰膝酸软，食欲不振\n无比山药丸\n肾阴不足，湿热留恋腰膝酸软，手足心热，头晕耳鸣，脉细数\n知柏地黄丸\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n诊断公式\n慢性原发/继发性肾脏疾病病史+肾小球滤过率<90mL/(min·1.73m2),或Ccr<80mL/min、Sar>\n133umol/L\n本虚证-脾肾气虚\n乏力，气短懒言，纳呆腹胀，腰酸膝软，便溏\n六君子汤\n本虚证-脾肾阳虚\n神疲乏力，纳差便溏，腰部冷痛，畏寒肢冷\n济生肾气丸\n本虚证-气阴两虚\n神疲乏力，手足心热，脉沉细\n参芪地黄汤\n本虚证-肝肾阴虚\n头晕头痛，两目干涩，腰膝酸软，脉弦\n杞菊地黄汤\n慢性肾衰\n竭\n本虚证-阴阳两虚\n畏寒肢冷，手足心热，口干欲饮，五更泄泻\n金匮肾气丸\n辨证论治\n标实证-湿浊\n恶心呕吐，胸闷纳呆，口有尿味\n小半夏加茯苓汤\n口干口苦，恶心频频，苔黄腻。\n标实证-湿热\n中焦-黄连温胆汤；\n小溲黄赤维人医考\n下焦-四妙丸\n标实证-水气\n水肿，胸水、腹水\n五苓散\n标实证-血瘀\n面色晦暗，舌紫暗有瘀点瘀斑，脉细涩\n桃红四物汤\n标实证-肝风\n头痛头晕，手足蠕动，筋惕肉暇，抽搐痉厥\n天麻钩藤饮\n五、血液及造血系统疾病\n诊断公式\n慢性失血病史+皮肤、黏膜苍白、异食癖、匙状甲+血清铁↓、Hb↓+铁剂治疗有效\n脾胃虚弱\n神疲乏力，食少便溏，脉细弱\n香砂六君子汤合当归补血汤\n缺铁性贫\n血\n心脾两虚\n倦怠乏力，头晕目眩，心悸失眠，食欲不振\n归脾汤或八珍汤\n辨证论治\n脾肾阳虚\n形寒肢冷，腰膝酸软，便溏，小便清长，阳痿\n八珍汤合无比山药丸\n虫积\n嗜食生米、泥土、茶叶\n化虫丸合八珍汤\n诊断公式\n贫血、感染、出血+无肝脾肿大+三系细胞减少、网织红细胞减少+骨髓象示增生减低\n肾阴虚\n颧红盗汗，腰膝酸软，舌红少苔脉细数\n左归丸合当归补血汤\n肾阳虚\n形寒肢冷，气短懒言，大便稀溏，脉细无力\n右归丸合当归补血汤\n再生障碍\n肾阴阳两虚\n倦怠乏力，手足心热，腰膝酸软，畏寒肢冷\n左归丸、右归丸合当归补血汤\n性贫血\n辨证论治\n肾虚血瘀\n面色晦暗，腰膝酸软，皮肤紫斑，脉涩\n六味地黄丸或金匮肾气丸合桃红\n四物汤\n气血两虚\n面白无华，唇淡，头晕心悸，气短乏力，脉细弱\n八珍汤\n热毒壅盛\n壮热，口渴，舌红而干，苔黄，脉洪数\n清瘟败毒饮\n1.白细胞减少症=外周血白细胞数持续<4.0×10°/L\n诊断公式\n2.粒细胞减少症=中性粒细胞绝对数<2.0×10°L\n白细胞减\n3.粒细胞缺乏症=中性粒细胞绝对数<0.5×10°/L\n少症和粒\n气血两虚\n面色萎黄，头晕目眩，倦怠乏力，脉细弱\n归脾汤\n细胞缺乏\n脾肾亏虚\n腰膝酸软，纳少便溏，畏寒肢冷，脉沉细或沉迟\n黄芪建中汤合右归丸\n症\n辨证论治\n气阴两虚\n疲倦乏力，五心烦热，盗汗或自汗\n生脉散\n肝肾阴虚\n腰膝酸软，头晕耳鸣，舌红少苔，脉细数\n六味地黄丸\n外感温热\n发热，口渴欲饮，舌红绛，苔黄，脉滑数或细数\n犀角地黄汤合玉女煎\n诊断公式\n贫血、发热、出血+肝脾肿大、胸骨下端局部压痛、淋巴结肿大+骨髓原始细胞≥20%\n热毒炽盛\n壮热烦躁，头痛面赤，咽喉肿痛，脉大\n黄连解毒汤合清营汤\n急性白血\n辨证论治\n痰热瘀阻\n痰多，发热，舌质紫暗，苔黄腻\n温胆汤合桃红四物汤\n病\n(助理不\n阴虚火旺\n五心烦热，盗汗，舌质红苔黄，脉细数\n知柏地黄丸合二至丸\n考)\n气阴两虚\n气短，乏力，手足心热，脉沉细\n五阴煎\n湿热内蕴\n头身困重，肛门灼热，舌红苔黄腻，脉滑数\n葛根芩连汤\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyivikao123\n诊断公式\n胸骨压痛、脾肿大+BCR/ABL融合基因(Ph染色体)、白细胞数1、NAP活性↓或阴性\n阴虚内热\n手足心热，舌质光红，苔少，脉细数\n青蒿鳖甲汤\n慢性髓细\n胞白血病\n瘀血内阻\n面色晦暗，皮肤瘀斑，舌质紫暗，脉细涩\n膈下逐瘀汤\n辨证论治\n气血两虚\n头晕眼花，疲乏无力，气短懒言，脉细弱\n八珍汤\n热毒壅盛\n壮热汗出，口渴，衄血发斑，舌红苔黄脉数\n清营汤合犀角地黄汤\n诊断公式\n广泛皮肤、黏膜出血+血小板数量减少+骨髓增生活跃：巨核细胞增多，成熟障碍\n原发免疫\n血热妄行\n色泽新鲜，发热，口渴，便秘尿黄，脉滑数\n犀角地黄汤\n性血小板\n阴虚火旺\n耳鸣，低热颧红，盗汗，脉细数\n玉女煎\n减少症\n辨证论治\n气不摄血\n气短，头晕目眩，面色萎黄，脉弱\n归脾汤人医号\n瘀血内阻\n血色紫暗，面色黧黑，舌质紫暗有瘀斑、瘀点\n桃红四物汤\n诊断公式\n贫血、出血、感染+病态造血、细胞遗传学异常、病理学改变\n骨髓增生\n气血两虚\n面色萎黄，唇甲色淡，气短无力，脉虚无力\n八珍汤\n异常综合\n气阴两虚\n气短懒言，口干舌燥，五心烦热，潮热盗汗\n大补元煎\n征(助理不辨证论治\n阴虚内热\n五心烦热，虚烦不眠，盗汗，脉象细数\n清骨散\n考)\n阴阳两虚\n畏寒肢冷，腰膝酸软，午后低热，盗汗，脉沉细\n右归丸合左归丸\n瘀毒内阻\n皮肤瘀斑，肋下癫积，舌质紫暗，脉细涩\n桃仁红花煎\n更多电子版资料扫码关注\n1.中西L\n新版大纲资料整理更新\n六、内分泌与代谢疾病\n2.中西医外\n3.中西恶妇\n4针负总论\n54针奥主穴\n三病方\n料病证方\n病方\n第考+特注\n影决配大\n六激读\n规津\nP\n回\n2\n6-四医诊断\n7-中药相同\n8-方市执业\n9中套功效\n10-方加图\n学重连\n功效药物总1.2.瑛\n速抄\n理1.23\n点整理\n结\n方刻\n方\n诊断公式\n高代谢症状和体征+甲状腺肿大、眼征+血清甲状腺激素水平升高，TSH↓\n气滞痰凝\n烦躁易怒，两胁胀满，苔白腻，脉弦滑\n逍遥散合二陈汤\n甲状腺功\n能亢进症\n肝火旺盛\n烦躁易怒，面红烘热，口苦，便秘，脉弦数\n辨证论治\n龙胆泻肝汤\n阴虚火旺\n口干咽燥，五心烦热，舌红少苔，脉细数\n天王补心丹\n气阴两虚\n神疲乏力气短，手足心热，脉细数无力\n生脉散\n诊断公式\n甲状腺手术史、药物或放射治疗史+畏寒、乏力、表情淡漠、反应迟钝+FT↓、TSHT\n脾肾气虚\n神疲乏力，少气懒言，纳呆，腰膝酸软，便溏\n四君子汤合大补元煎\n甲状腺功\n能减退症\n脾肾阳虚\n乏力，畏寒肢冷，腰膝酸软，阳痿，脉沉细缓\n脾阳虚：附子理中丸；\n辨证论治\n肾阳虚：右归丸\n心肾阳虚\n形寒肢冷，心悸胸闷，腰膝酸软，阳痿，脉迟缓\n真武汤合苓桂术甘汤\n阳气衰微\n嗜睡昏迷，呼吸微弱，脉微欲绝\n四逆加人参汤+参附注射液\n诊断公式\n甲状腺肿大、结节、疼痛、压痛+全身症状+甲状腺摄\"1率和血清T?、T呈分离现象\n亚急性甲\n肝胆郁热\n发热，心悸易怒，舌红，苔薄黄，脉浮数或弦数龙胆泻肝汤\n状腺炎(助\n阴虚火旺\n理不考)\n辨证论治\n口燥咽干，潮热盗汗，舌红，苔少，脉细数\n清骨散\n痰瘀互结\n疼痛不移，舌质紫暗，或瘀斑瘀点，脉细涩\n海藻玉壶汤\n脾阳不振\n疲乏无力，畏寒肢冷，纳呆腹胀，脉沉细\n实脾饮\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyiyikao123\n诊断公式\n慢性淋巴\n中年妇女+甲状腺弥漫性对称性肿大+TPOAb及TgAb 明显增高\n细胞性甲\n痰瘀凝结\n甲状腺疼痛，舌质暗，有瘀斑，苔白腻，脉细涩\n二陈汤合桃红四物汤\n状腺炎(助\n肝郁脾虚\n胸胁苦闷，善太息，纳差便溏，脉弦滑\n逍遥散\n辨证论治\n理不考)\n肝肾阴虚\n腰膝酸软，头晕目眩，舌质红，苔少，脉细数\n杞菊地黄丸\n脾肾阳虚\n面色咣白，纳呆便溏，畏寒肢冷，腰膝酸软\n四逆汤合五苓散\n诊断公式\n多饮、多食、多尿、消瘦+空腹血浆葡萄糖≥7.0mol/L或0GTT 2h 血糖或随机血糖≥11.1mmol/L\n阴虚燥热证-上消\n(肺热伤津证)\n烦渴多饮，多汗，舌边尖红，脉洪数\n消渴方\n阴虚燥热证-中消\n(胃热炽盛证)\n多食易饥，形体消瘦，苔黄，脉滑实有力\n玉女煎\n阴虚燥热证-下消\n(肾阴亏虚证)\n尿频混浊如脂膏，腰膝酸软，脉细数\n六味地黄丸\n糖尿病\n辨证论治\n气阴两虚证\n能食与便溏并见，苔白而干，脉弱\n七味白术散\n阴阳两虚证\n耳轮焦干，腰膝酸软，形寒畏冷，阳痿\n金匮肾气丸\n痰瘀互结证\n形体肥胖，四肢沉重或刺痛，舌有瘀斑\n平胃散合桃红四物汤\n脉络瘀阻证\n面色晦暗，肢体刺痛，舌暗或有瘀斑，脉沉涩\n血府逐瘀汤\n并发症-疮痈\n易发生疮疡痈疽，高热神昏，舌红苔黄，脉数\n五味消毒饮合黄芪六一散\n并发症-白内障、雀视物模糊，渐至昏蒙，直至失明；夜间不能视\n目、耳聋\n物，白昼正常；耳鸣、耳聋\n杞菊地黄丸、羊肝丸、磁朱丸\n诊断公式\n糖尿病病史或不良饮食习惯+黄色瘤、肥胖+TC≥6.2mmol/L、TG≥2.3mmol/L、LDL-C≥4,1mmol/L、HDL-C\n<1.0mmol/L\n胃热滞脾\n消谷善饥，胃脘灼痛，舌红苔黄腻，脉弦滑\n保和丸合小承气汤\n气滞血瘀\n胸部憋气或胸部刺痛，舌质紫暗，脉弦\n血府逐瘀汤合失笑散\n血脂异常\n痰浊中阻\n形体肥胖，腹胀纳呆，苔白腻，脉滑\n导痰汤\n辨证论治\n肝肾阴虚\n五心烦热，腰膝酸软，舌红苔少脉细数\n杞菊地黄汤\n脾肾阳虚\n畏寒肢冷，腰膝腿软，便溏，腹胀纳呆\n附子理中汤\n肝郁脾虚\n心烦易怒，胸胁闷痛，纳食不香，脉弦细\n逍遥散\n诊断公式\n高尿酸血症(血尿酸>420umol/L)+拇趾及第一跖趾关节红肿热痛+滑囊液检查及痛风石中见尿酸盐\n结晶\n高尿酸血\n风寒湿阻\n游走性疼痛，或痛处不移，或肢体关节重着\n蠲痹汤\n症与痛风\n风湿热郁\n红肿热痛，遇热痛甚，发热，脉滑数\n白虎加桂枝汤\n辨证论治\n痰瘀痹阻\n皮下结节，舌质紫暗有瘀点、瘀斑\n桃红饮\n肝肾亏虚\n僵直畸形，腰膝酸痛，神疲乏力，脉细弱\n独活寄生汤\n七、风湿性疾病\n诊断公式\n腕关节/掌指关节/近端指间关节疼痛或压痛+对称性关节肿胀+RF阳性\n活动期-湿热痹阻\n下肢沉重酸胀，舌苔黄腻，脉滑数\n四妙丸\n类风湿关\n活动期-阴虚内热\n盗汗，手足心热，舌质干红少苔脉细数\n丁氏清络饮\n节炎\n辨证论治\n活动期-寒热错杂\n关节灼热疼痛，形寒肢凉，阴雨天疼痛加重\n桂枝芍药知母汤\n缓解期-痰瘀互结\n痛处不移，皮下结节，肢体顽麻\n身痛逐瘀汤合指迷茯苓丸\n缓解期-肝肾亏损\n肌肉萎缩，筋脉拘急，腰膝酸软无力，脉细弱\n独活寄生汤\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n诊断公式\n蝶形红斑或盘状红斑+光过敏+多系统病变+多种抗体阳性(抗Sm、抗dsDNA、ANA)\n气营热盛\n高热，满面红赤，舌红绛苔黄，脉洪数\n清瘟败毒饮\n阴虚内热\n低热，面色潮红，舌质红少苔，脉细数\n玉女煎合增液汤\n热郁积饮\n胸闷胸痛，咽干口渴，舌红苔厚腻，脉滑数\n葶苈大枣泻肺汤合泻白散\n系统性红\n斑狼疮\n瘀热痹阻\n手足瘀点累累，低热，烦躁多怒，脉涩数\n犀角地黄汤\n辨证论治\n脾肾两虚\n神疲乏力，畏寒肢冷，两腿浮肿，腰股俱肿\n济生肾气丸\n气血两亏\n健忘失眠，面色不华，脉细缓\n八珍汤\n脑虚瘀热\n身灼热，神昏谵语，舌謇，舌鲜绛，脉细数\n清宫汤送服安宫牛黄丸或至宝丹\n瘀热伤肝\n两胁胀痛，烦躁易怒，皮肤红斑、瘀斑，脉弦\n茵陈蒿汤合柴胡疏肝散\n八、神经系统疾病\n诊断公式\n详细准确发作过程和表现+脑电图痫性放电\n发作期-阳痫\n突然仆倒，喉中痰鸣，舌红苔黄腻，脉弦滑\n黄连解毒汤合定痫丸\n发作期-阴痫\n突然昏仆，手足清冷，脉沉细或沉迟\n五生丸合二陈汤\n癫痫\n辨证论治\n休止期-肝火痰热\n性情急躁，口苦咽干，时吐痰涎，脉弦滑数\n龙胆泻肝汤合涤痰汤\n休止期-脾虚痰湿\n神疲乏力，痰多，纳少便溏，脉濡弱\n醒脾汤\n休止期-肝肾阴虚\n头晕目眩，两目干涩，腰膝酸软，脉细数\n左归丸\n休止期-瘀阻清窍\n颜面口唇青紫，舌质紫暗或有瘀斑，脉涩\n通窍活血汤\n诊断公式\n50岁以上+不良嗜好+一过性意识障碍<1h,24小时内完全恢复+检查(-)\n短暂性脑\n肝肾阴虚、风阳上扰头晕目眩，目胀耳鸣，舌质红，脉弦或细数\n镇肝熄风汤\n缺血发作\n辨证论治\n气虚血瘀、脉络瘀阻肢体软弱无力，舌暗有瘀点，脉沉细无力或涩\n补阳还五汤\n痰瘀互结、阻滞脉络 头重如蒙，舌暗苔黄厚腻，脉滑数或涩\n黄连温胆汤合桃红四物汤\n1.动脉硬化性脑梗死=中老年人+多种血管性危险因素(高血压、动脉硬化、糖尿病)+偏瘫、偏深\n感觉障碍、失语、共济失调+脑CT低密度病灶或未见明显异常\n2.脑栓塞=青壮年+明显原发疾病和栓子来源(房颤、风心病、骨折史)+失语、偏瘫、偏深感觉障碍+\n脑 CT低密度病灶或未见明显异常\n诊断公式\n3.腔隙性梗死=中老年人+长期高血压病史+腔隙综合征+CT或MRI 检查+EEG、腰椎穿刺或DSA等(-)\n+预后良好\n4.脑出血=50岁以上+高血压病史+激动中发病+意识障碍、脑膜刺激征+偏瘫、失语+脑CT检查高密\n度阴影\n动脉硬化\n5.蛛网膜下腔出血=突发剧烈头痛、呕吐+脑膜刺激征阳性+CT示高密度影+脑脊液呈均匀血性\n性脑梗死、\n肝阳暴亢，风火上扰头晕头痛，耳鸣目眩，舌质红，苔黄，脉弦\n天麻钩藤饮\n脑栓塞、腔\n风痰瘀血，痹阻脉络肌肤不仁，舌质暗苔白腻，脉弦滑\n真方白丸子\n隙性梗死、\n痰热腑实，风痰上扰痰多，腹胀便秘，舌红，苔黄厚燥，脉弦滑\n星萎承气汤\n脑出血、蛛\n网膜下腔\n气虚血瘀\n软弱无力，气短声低，舌质淡暗或有瘀斑\n补阳还五汤\n出血\n阴虚风动\n耳鸣目眩，腰膝酸软，舌红苔黄，脉弦细而数\n镇肝熄风汤\n辨证论治\n脉络空虚，风邪入中恶寒发热，关节酸痛，苔薄白，脉浮弦或弦细\n大秦艽汤\n突然昏仆，口噤目张，气粗息高，两手握固，\n痰热内闭清窍\n大便干结，舌红，苔黄腻，脉弦滑数\n安宫牛黄丸+羚羊角汤\n痰湿壅闭心神\n突然昏仆，不省人事，牙关紧闭，口噤不开，\n四肢欠温，苔白滑而腻，脉沉维人医考\n涤痰汤\n突然昏仆，不省人事，目合口开，鼻鼾息微，\n元气败脱，心神涣散手撒肢冷，二便自遗，肢体软瘫，脉微欲绝\n大剂参附汤合生脉散\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n1.血管性痴呆=50～70岁+缺血性脑血管病史+认知功能下降+神经功能缺损症状和体征\n诊断公式\n2.Alzheimer 病=60岁以上+认知障碍+记忆力障碍、其他智能障碍+量表、辅助检查+病理诊断\n血管性痴\n髓海不足\n齿枯发焦，腰腿酸软，头晕耳鸣，脉沉细弱\n七福饮\n呆(助理不\n脾肾两虚\n腰膝酸软，食少纳呆，少气懒言，脉沉弱\n还少丹\n考)、\n痰浊蒙窍\n头晕重，苔腻，脉滑或濡\n涤痰汤\nAlzheimer\n辨证论治\n瘀阻脑络\n肌肤甲错，舌质暗或有瘀斑，脉细涩\n通窍活血汤\n病\n心肝火旺\n急躁易怒，面红目赤，舌质红，苔黄，脉弦数\n黄连解毒汤\n肝肾阴虚\n耳鸣，腰膝酸软，舌红少苔，脉细数\n知柏地黄丸\n心血不足\n面唇无华，纳呆食少，大便溏薄，脉细弱\n归脾汤\n诊断公式\n中老年，缓慢发展+动作迟缓+静止性震颤或肌强直+症状左右肢体不对称+左旋多巴治疗有效\n风阳内动\n面赤烦躁，舌质红，苔黄，脉弦\n天麻钩藤饮合镇肝熄风汤\n痰热风动\n口吐痰涎，舌红，舌苔黄腻，脉弦滑数\n导痰汤合羚角钩藤汤\n帕金森病\n辨证论治\n气血亏虚\n神疲乏力，动则气短，心悸健忘，脉沉濡无力\n人参养荣汤\n髓海不足\n腰膝酸软，善忘，神呆、痴傻\n龟鹿二仙膏\n阳气虚衰\n畏寒肢冷，动则气短，自汗，脉沉迟无力\n地黄饮子\n医\n扫码关注\n扫码进入小程序\n如有任何问题\n更多电子版资料扫码关注\n1-中西医儿\n十、内科常见危重症\n了解更多考试资讯\n获取更多\n扫码添加老师咨询\n新版大纲资料整理更新\n2中西医外\n3.中西医妇\n4针负总总\n5-牡负主穴\n科病证方\n科病证方\n所证方\n宽考+畅定\n数决和配六\n六影决\n规律\nP\n6-四需沙E\n7-中药相网\n8-方招沈业9中药功效\n10-方助\n学重进考\n功效药物总\n1.2 类\n速记小\n理1. 23\n点整\n结\n方相\n方\n诊断公式\n意识异常+脉搏超过100次/分或者不能触及+四肢湿冷，尿少+收缩压<80mHg+脉压<20mmHg\n气阴耗伤\n心烦口渴，脉细数无力，或见脉散大\n生脉散\n真阴衰竭\n汗出如油，身热心烦，舌光干枯无苔\n三甲复脉汤\n休克\n阳气暴脱\n辨证论治\n冷汗淋漓，四肢厥冷，脉微欲绝或不能触及\n四逆汤\n热毒炽盛\n壮热，口渴，舌红苔黄燥，脉沉细而数或沉数\n黄连解毒汤\n气滞血瘀\n口唇青紫，皮肤瘀斑，舌暗紫\n四逆散合血府逐瘀汤\n心气不足\n怔忡不安，气短而促，脉细而促或结、代\n炙甘草汤\n十一、肺系病证\n辨病要点\n呼吸困难，甚至张口抬肩，鼻翼扇动，不能平卧\n实喘-风寒壅肺\n恶寒，或有发热，无汗，脉浮紧\n麻黄汤合华盖散\n喘证\n实喘-表寒肺热\n形寒，身热，舌边红，脉浮数或滑\n麻杏石甘汤\n辨证论治\n实喘-痰热郁肺\n咳嗽痰多，舌质红，舌苔薄黄或腻，脉滑数\n桑白皮汤\n实喘-痰浊阻肺\n痰多黏腻色白，苔白腻，脉象滑或濡\n二陈汤合三子养亲汤\n实喘-肺气郁痹\n每遇情志刺激而诱发+平素常多忧思抑郁+脉弦\n五磨饮子\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyiyikao123\n虚喘-肺气虚耗\n喘促短气，气怯声低+自汗畏风人医考\n生脉散合补肺汤\n虚喘-肾虚不纳\n喘促日久，动则喘甚，呼多吸少，气不得续\n金匮肾气丸合参蛤散\n虚喘-正虚喘脱\n端坐不能平卧，稍动则咳喘欲绝，脉浮大无根\n参附汤送服黑锡丹\n十二、心系病证\n辨病要点\n入寐困难或寐而易醒，醒后不寐，连续三周以上\n肝火扰心\n急躁易怒，目赤耳鸣，舌红苔黄，脉弦而数\n龙胆泻肝汤\n不寐\n痰热扰心\n胸闷脘痞，舌偏红，苔黄腻，脉滑数\n黄连温胆汤\n辨证论治\n心脾两虚\n心悸健忘，四肢倦怠，腹胀便溏，面色少华\n归脾汤\n心肾不交\n腰膝酸软，潮热盗汗，舌红少苔，脉细数\n六味地黄汤合黄连阿胶汤\n心胆气虚\n触事易惊，终日惕惕，胆怯心悸\n安神定志丸合酸枣仁汤\n辨病要点\n自觉心中悸动不安，甚至不能自主\n心虚胆怯\n善惊易恐，坐卧不安\n安神定志丸\n心血不足\n失眠健忘，面色无华\n归脾汤\n心悸\n心阳不振\n动则尤甚，面色苍白，形寒肢冷\n桂枝甘草龙骨牡蛎汤合参附汤\n辨证论治\n水饮凌心\n胸闷痞满，面浮肢肿，舌淡胖，苔白滑\n苓桂术甘汤\n阴虚火旺\n五心烦热，盗汗，舌红少津，苔少，脉细数\n天王补心丹\n心脉瘀阻\n痛如针刺，舌质紫暗或有瘀斑，脉涩\n桃仁红花煎\n痰火扰心\n大便秘结，小便短赤，舌红，苔黄腻，脉弦滑黄连温胆汤\n十三、脾系病证\n辨病要点\n心下痞塞，胸膈胀满，触之无形，按之柔软，压之无痛\n实痞-饮食内停\n嗳腐吞酸，矢气频作，味臭如败卵\n保和丸\n实痞-痰湿中阻\n身重困倦，呕恶纳呆，苔白厚腻，脉沉滑\n二陈平胃汤\n胃痞\n辨证论治\n实痞-湿热阻胃\n口干不欲饮，舌红苔黄腻，脉滑数\n泻心汤合连朴饮\n实痞-肝胃不和\n胸胁胀满，心烦易怒，善太息，脉弦\n越鞠丸合枳术丸\n虚痞-脾胃虚弱\n纳呆便溏，神疲乏力，语声低微\n补中益气汤\n虚痞-胃阴不足\n嘈杂，饥不欲食，舌红少苔，脉细数\n益胃汤\n辨病要点\n胃脘以下、耻骨毛际以上部位的疼痛\n寒邪内阻\n腹痛遇寒痛甚，得温痛减+形寒肢冷，脉沉紧\n良附丸合正气天香散\n湿热壅滞\n便秘，或溏滞不爽，苔黄燥或黄腻，脉滑数\n大承气汤\n腹痛\n饮食积滞\n辨证论治\n脘腹胀满疼痛，拒按，嗳腐吞酸\n枳实导滞丸\n肝郁气滞\n胀闷，痛无定处，兼痛窜两胁+遇忧思恼怒则剧柴胡疏肝散\n瘀血内停\n腹痛较剧，痛如针刺，舌质紫暗，脉细涩\n少腹逐瘀汤\n中虚脏寒\n腹痛绵绵，时作时止，喜温喜按，形寒肢冷\n小建中汤\n辨病要点\n排便次数增多，粪质稀溏或完谷不化，甚至泻出如水样\n寒湿内盛\n泄泻清稀如水样，外感风寒，恶寒，发热\n藿香正气散\n湿热伤中\n泻下急迫，粪色黄褐，气味臭秽，苔黄腻\n葛根芩连汤\n泄泻\n食滞肠胃\n辨证论治\n腹痛肠鸣，泻下粪便臭如败卵，嗳腐酸臭\n保和丸\n肝气乘脾\n因抑郁恼怒，或情绪紧张之时而作\n痛泻要方\n脾胃虚弱\n迁延反复，进油腻食物，则大便次数增加\n参苓白术散\n肾阳虚衰\n黎明前脐腹作痛，完谷不化，形寒肢冷，腰膝酸软\n四神丸\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n辨病要点\n排便间隔时间长；大便粪质干结，排出艰难，或欲大便而艰涩不畅\n实秘-热秘\n口干口臭，面红心烦，舌红，苔黄燥，脉滑数\n麻子仁丸\n实秘-气秘\n欲便不得出，肠鸣矢气，腹中胀痛，脉弦\n六磨汤\n便秘\n实秘-冷秘\n腹痛拘急，手足不温，舌苔白腻，脉弦紧\n大黄附子汤\n辨证论治\n虚秘-气虚秘\n有便意，但排便困难，汗出短气，便后乏力\n黄芪汤\n虚秘-血虚秘\n大便干结，面色无华，口唇色淡\n润肠丸\n虚秘-阴虚秘\n大便干结，如羊屎状，舌红少苔，脉细数\n增液汤\n虚秘-阳虚秘\n面色觥白，四肢不温，腹中冷痛\n济川煎\n十四、肝胆病证\n辨病要点\n一侧或两侧胁肋部疼痛\n肝郁气滞\n胀痛走窜不定，因情志变化而增减，嗳气频作\n柴胡疏肝散\n胁痛\n肝胆湿热\n胁肋重着，身目发黄，舌红苔黄腻，脉弦滑数\n龙胆泻肝汤\n辨证论治\n瘀血阻络\n胁肋刺痛，痛有定处，舌质紫暗，脉沉涩\n血府逐瘀汤或复元活血汤\n肝络失养\n隐痛，悠悠不休，舌红少苔，脉细弦而数\n一贯煎\n辨病要点\n目黄、身黄、小便黄\n阳黄-热重于湿\n色泽鲜明，苔黄腻，脉象弦滑或滑数\n茵陈蒿汤\n阳黄-湿重于热\n色不甚鲜明，头重身困，脉濡缓或弦滑\n茵陈四苓散\n黄疸\n阳黄-胆腑郁热\n身目黄染，右胁疼痛，牵引肩背，寒热往来\n大柴胡汤\n辨证论治\n阳黄-热毒炽盛(急黄)\n黄疸迅速加深，其色金黄鲜明+神昏谵语\n犀角散\n阴黄-寒湿困脾\n黄色晦暗，或如烟熏，苔白腻，脉濡缓\n茵陈术附汤\n阴黄-脾虚血亏\n淡黄，肢软乏力，心悸气短，便溏，脉虚\n黄芪建中汤\n辨病要点\n腹内结块，或痛或胀，结块固定不移，痛有定处\n积证\n气滞血阻\n腹部积块质软不坚，固定不移，脉弦\n大七气汤\n辨证论治\n瘀血内结\n刺痛，形体消瘦，舌质紫有瘀斑瘀点，脉细涩\n膈下逐瘀汤合六君子汤\n正虚瘀结\n久病体弱，积块坚硬，神倦乏力，脉弦细\n八珍汤合化积丸\n辨病要点\n腹中结块，或痛或胀，聚散无常，痛无定处\n聚证\n肝郁气滞\n攻窜胀痛，常随情绪波动而起伏，脉弦\n逍遥散\n辨证论治\n食滞痰阻\n腹部时有条索状物聚起，苔腻，脉弦滑\n六磨汤\n辨病要点\n头部疼痛\n外感-风寒头痛\n头痛连及项背，恶风畏寒+苔薄白，脉浮紧\n川芎茶调散\n外感-风热头痛\n头胀如裂，发热或恶风，苔薄黄，脉浮数\n芎芷石曹汤\n外感-风湿头痛\n头痛如裹，肢体困重，苔白腻，脉濡\n羌活胜湿汤\n头痛\n内伤-肝阳头痛\n头昏胀痛，胁痛，舌红苔黄，脉弦数\n天麻钩藤饮\n辨证论治\n内伤-血虚头痛\n头痛隐隐，缠绵不休，面色少华，神疲乏力\n加味四物汤\n内伤-痰浊头痛\n头痛昏蒙，胸脘满闷，苔白腻，脉滑或弦滑\n半夏白术天麻汤\n内伤-肾虚头痛\n头痛且空，眩晕耳鸣，腰膝酸软\n大补元煎\n内伤-瘀血头痛\n痛处固定不移，舌紫暗，或有瘀斑、瘀点\n通窍活血汤\n内伤-气虚头痛\n头痛隐隐，遇劳加重，神疲乏力，气短懒言\n益气聪明汤\n辨病要点\n头晕目眩，视物旋转\n肝阳上亢\n急躁易怒，口苦，目赤，脉弦细而数\n天麻钩藤饮或羚羊角汤\n眩晕\n气血亏虚\n动则加剧，气短声低，面色少华，心悸失眠\n八珍汤\n辨证论治\n肾精不足\n腰酸膝软，耳鸣，齿摇，脉弦细或弱或细数\n河车大造丸\n痰浊内蕴\n眩晕，头重如蒙，胸闷，呕吐痰涎，脉弦滑\n半夏白术天麻汤\n瘀血阻窍\n面唇紫暗，舌暗有瘀斑，脉涩\n通窍活血汤\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX:zhongyiyikao123\n十五、肾系病证\n辨病要点\n眼睑、头面、四肢、腹背，甚至全身浮肿\n阳水-风水泛滥\n眼睑浮肿，继则全身皆肿，恶寒发热，脉浮越婢加术汤\n阳水-湿毒浸淫\n眼睑浮肿，延及全身，身发疮痍，甚则溃烂麻黄连翘赤小豆汤合五味消毒饮\n阳水-水湿浸渍\n下肢明显，按之没指，苔白腻，脉沉缓\n五皮饮合胃苓汤\n水肿\n辨证论治\n阳水-湿热壅盛\n烦热口渴，溲赤，苔黄腻，脉沉数或濡数\n疏凿饮子\n阴水-脾阳虚衰\n腰以下为甚，按之凹陷不易恢复，纳呆便溏，\n面色萎黄，神疲乏力\n实脾饮\n阴水-肾阳衰微\n腰以下甚，按之凹陷不起，腰部冷痛酸重\n济生肾气丸合真武汤\n阴水-瘀水互结\n皮肤瘀斑，腰部刺痛，舌紫暗脉细涩\n桃红四物汤合五苓散\n十六、气血津液病证\n辨病要点\n心情抑郁，情绪不宁，胸部满闷，胁肋胀痛，或易怒喜哭，或咽中如有异物梗塞\n肝气郁结\n精神抑郁，胁肋胀痛，痛无定处，脉弦\n柴胡疏肝散\n气郁化火\n急躁易怒，口苦而干，目赤，苔黄，脉弦数\n丹栀逍遥散\n痰气郁结\n咽中如有物梗塞，吞之不下，咯之不出\n半夏厚朴汤\n郁证\n心神失养\n心神不宁，多疑易惊，悲忧善哭，喜怒无常\n甘麦大枣汤\n辨证论治\n心脾两虚\n心悸胆怯，失眠健忘，纳差\n归脾汤\n心阴亏虚\n心悸，健忘，失眠，多梦，五心烦热，盗汗\n天王补心丹\n气滞血瘀\n胸胁疼痛，舌质紫暗或有瘀点、瘀斑，脉弦涩\n血府逐瘀汤\n肝肾阴虚\n眩晕耳鸣，筋惕肉暇，舌干红，脉弦细或数\n杞菊地黄丸\n1.鼻衄：血自鼻道外溢\n2.齿衄：血自齿龈或齿缝外溢\n3.咳血：血由肺、气道而来，经咳嗽而出，或觉喉痒胸闷，一咯即出\n辨病要点\n4.吐血：血随呕吐而出，常伴有食物残渣等胃内容物\n5.便血：大便色鲜红、暗红或紫暗，甚至黑如柏油样，次数增多\n6.尿血：小便中混有血液或夹有血丝，排尿时无疼痛\n7.紫斑：肌肤出现青紫斑点，小如针尖，大者融合成片，压之不退色\n热邪犯肺\n身热，恶风，咳嗽，痰少\n桑菊饮\n胃热炽盛\n口渴欲饮，鼻干，口干臭秽，烦躁，便秘\n玉女煎\n鼻衄\n肝火上炎\n烦躁易怒，两目红赤，口苦，脉弦数\n龙胆泻肝汤\n气血亏虚\n血色淡红+神疲乏力，面色无华\n归脾汤\n胃火炽盛\n齿龈红肿疼痛，口臭\n加味清胃散合泻心汤\n血证\n齿衄\n阴虚火旺\n齿摇不坚，舌质红，苔少，脉细数\n六味地黄丸合茜根散\n燥热伤肺\n喉痒咳嗽，痰中带血，口干鼻燥\n桑杏汤\n咳血\n肝火犯肺\n胸胁胀痛，烦躁易怒\n泻白散合黛蛤散\n辨证论治\n阴虚肺热\n颧红，潮热盗汗，舌质红，脉细数\n百合固金汤\n胃热壅盛\n夹有食物残渣，口臭，便秘\n泻心汤合十灰散\n吐血\n肝火犯胃\n口苦胁痛，心烦易怒》人医考\n龙胆泻肝汤\n气虚血溢\n吐血缠绵不止，神疲乏力\n归脾汤\n肠道湿热\n便血色红黏稠，舌质红，苔黄腻，脉濡数\n地榆散合槐角丸\n便血\n气虚不摄\n食少，体倦，面色萎黄\n归脾汤\n脾胃虚寒\n便血紫暗，甚则黑色，腹部隐痛，喜热饮\n黄土汤\n下焦湿热\n尿血\n小便黄赤灼热，尿血鲜红，面赤口疮，脉数\n小蓟饮子\n肾虚火旺\n颧红潮热，腰膝酸软，舌质红，脉细数\n知柏地黄丸\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyivikao123\n脾不统血\n久病尿血，食少，体倦乏力\n归脾汤\n肾气不固\n久病尿血，精神困惫，腰脊酸痛\n无比山药丸\n血热妄行\n发热，口渴，便秘，舌红苔黄，脉弦数\n犀角地黄汤\n紫斑\n阴虚火旺\n时发时止+潮热，盗汗，舌质红，苔少，脉细数茜根散\n气不摄血\n反复发生肌衄，神疲乏力\n归脾汤\n1.痰饮：心下满闷，呕吐清水痰涎，胃肠沥沥有声，形体昔肥今瘦\n辨病要点\n2.悬饮：胸胁胀满，咳唾引痛，喘促不能平卧\n3.溢饮：身体疼痛而沉重，甚则肢体浮肿，当汗出而不汗出\n4.支饮：咳逆倚息，短气不得平卧，其形如肿\n脾阳虚弱\n胃中有振水音，脘腹喜温畏冷，泛吐清水痰涎\n苓桂术甘汤合小半夏加茯苓汤\n痰饮\n饮留胃肠\n自利，利后反快，心下续坚满，便秘，口干\n甘遂半夏汤或己椒苈黄丸\n痰饮\n邪犯胸肺\n寒热往来，身热起伏，呼吸、转侧疼痛加重\n柴枳半夏汤\n饮停胸胁\n辨证论治\n悬饮\n侧肋间胀满，甚则病侧胸廓隆起\n椒目瓜萎汤合十枣汤\n络气不和\n胸胁疼痛，如灼如刺，病侧胸廓变形，舌质暗香附旋覆花汤\n阴虚内热\n口干咽燥，颧红，心烦，舌质红少苔，脉细数\n沙参麦冬汤合泻白散\n溢饮\n恶寒，无汗，痰多白沫，胸闷，干呕\n小青龙汤\n寒饮伏肺\n痰吐白沫量多+遇寒即发，苔白滑，脉弦紧\n小青龙汤\n支饮\n脾肾阳虚\n怯寒肢冷，脐下动悸，小便不利，足跗浮肿\n金医肾气丸合苓桂术甘汤\n1.自汗：时时汗出，动则益甚者\n辨病要点\n2.盗汗：寐则汗出，醒来则止\n3.脱汗：在病情危重时全身大汗淋漓，或汗出如油\n4.黄汗：汗出色黄，染衣着色\n营卫不和\n汗出恶风，周身酸楚，或兼微发热，脉浮或缓\n桂枝汤\n肺气虚弱\n汗证\n自汗\n平时不耐风寒，易于感冒，体倦乏力，脉细弱\n玉屏风散\n心肾亏虚\n胸闷气短，腰酸腿软，面白唇淡，脉沉细\n芪附汤\n热郁于内\n辨证论治\n蒸蒸汗出，面赤，发热，气粗口渴，舌红苔黄厚\n竹叶石膏汤\n心血不足\n盗汗\n心悸怔忡，失眠多梦，面色少华，口唇色淡\n归脾汤\n阴虚火旺\n五心烦热，午后潮热，舌质红少津，少苔\n当归六黄汤\n脱汗\n病情危重，大汗淋漓，汗出如油，四肢厥冷\n参附汤\n黄汗\n汗出色黄，染衣着色。舌质红苔黄腻，脉弦滑\n龙胆泻肝汤\n辨病要点\n低热，或自觉发热，而体温并不升高\n阴虚发热\n午后潮热，或夜间发热，舌质红，脉细数\n清骨散\n血虚发热\n热势多为低热，头晕眼花，面白少华，唇甲色淡归脾汤\n内伤发热\n气虚发热\n劳累后发作或加剧，乏力气短，自汗，食少便溏\n补中益气汤\n辨证论治\n阳虚发热\n发热而欲近衣，形寒怯冷，四肢不温，面色觥白\n金匮肾气丸\n气郁发热\n势常随情绪波动而起伏，胁胀易怒，脉弦数\n丹栀逍遥散\n痰湿郁热\n胸闷脘痞，渴不欲饮，舌苔白腻或黄腻，脉濡数\n黄连温胆汤合中和汤\n血瘀发热\n午后或夜晚发热，舌质青紫或有瘀点、瘀斑\n血府逐瘀汤\n辨病要点\n突然昏倒，不省人事，或伴有四肢逆冷\n实证\n情志刺激诱发，突然昏倒，不知人事，脉沉弦\n通关散吹鼻醒神+五磨饮子\n气厥\n虚证\n眩晕昏仆，面色苍白，气息低微，汗出肢冷\n独参汤或四味回阳饮\n厥证(助理\n实证\n因急躁恼怒而发，面红目赤，舌暗红\n羚角钩藤汤或通瘀煎\n不考)\n辨证论治\n血厥\n虚证\n因失血过多而发，面色苍白，口唇无华州人医\n先用独参汤以固脱，继服人\n参养荣汤或当归补血汤\n痰厥\n素有咳喘宿痰，多湿多痰，喉有痰声\n导痰汤\n暑厥\n暑热夏季或高温环境，面红身热，汗出，舌红\n先用紫雪丹醒神开窍，继用\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n懒人医考整理\n搜索添加 获取更多资料VX: zhongyiyikao123\n干，脉洪数\n白虎加人参汤\n十七、肢体经络病证\n辨病要点\n肢体筋脉弛缓，软弱无力，日久不能随意运动而致肌肉萎缩\n热毒炽盛，气血两燔\n壮热，烦躁不宁，舌质红绛苔黄燥，脉洪数\n清瘟败毒饮\n痿证\n肺热津伤，筋失濡润呛咳少痰，咽干不利，舌红苔黄，脉细数\n清燥救肺汤\n辨证论治\n湿热浸淫，气血不运肢体困重，痿软无力，苔黄腻，脉细数\n加味二妙散\n脾胃亏虚，精微不运\n神疲乏力，气短，食少，便溏\n参苓白术散合补中益气汤\n肝肾亏损，髓枯筋痿下肢明显，腰膝酸软，舌红少苔，脉细数\n大补阴煎\n辨病要点\n腰部一侧或两侧疼痛\n寒湿腰痛\n冷痛重着，寒冷和阴雨天则加重，苔白腻\n甘姜苓术汤\n湿热腰痛\n痛处伴有热感，暑湿阴雨天症状加重，苔黄腻\n四妙丸\n腰痛\n辨证论治\n瘀血腰痛\n腰痛如刺，痛有定处，舌质暗紫，有瘀斑\n身痛逐瘀汤\n肾虚腰痛\n酸软无力，舌红少苔，脉弦细数；腰部冷痛，\n偏阴虚一左归丸；\n面色觥白，肢冷畏寒，脉沉细无力\n偏阳虚一右归丸；\n无明显阴阳偏虚一青蛾丸\n扫码关注\n了解更多考试资讯\n扫码进入小程序\n获取更多\n如有任何问题\n扫码添加老师咨询\n更多电子版资料扫码关注\n新版大纲资料整理更新\n1中西医儿\n2.中西医外\n3中妇\n4针负总给\n5针负主共\n科病证方\n科病证方\n科病证方\n常考+特定\n数决和配六\n六款决\n规律\n6-西落动断\n7-中药相网\n8-方刻业\n9-中药功效\n10-方市助\n学重维点考\n功效药物总\n1.2.美\n选记小抄\n理1.2.3\n卓整理\n结\n方剂\n方\n关注中西医考试 公众号，获取更多备考资讯\n懒人医考 祝你考试顺利\n"

// 与生产端 api/generate.ts 保持一致的 system prompt（全量注入）
const SYSTEM_PROMPT = `你是一位资深的中西医执业医师考试辅导专家，严格依据2026年中西医执业医师考试官方大纲。

【最高优先级原则】
你必须严格依据下方"考试资料知识库"生成中医内容（证型、选方、辨证秒杀、治法），不得自行编造或更改。

- 如果用户输入的疾病在知识库中能找到对应条目，必须**完全使用知识库中列出的证型数量和对应选方**，一个不能多，一个不能少。
- 知识库由 PDF 表格提取，部分疾病名或方剂名可能因表格换行被拆成两行（如"急性上呼吸\n道感染"应为"急性上呼吸道感染"，"柴胡疏肝散合胃\n苓汤"应为"柴胡疏肝散合胃苓汤"），你需要根据上下文合并还原为完整名称。
- syndromes 数组与 formulaRows 数组的证型必须**完全一致**（数量和内容都对齐知识库）。
- 辨证秒杀（symptom 字段）使用知识库中该证型对应的"辨证秒杀"描述。
- 如果用户输入的疾病在知识库中找不到，则按考试大纲通用知识生成，但要明确在 memoryCard.definition 中标注"该疾病不在官方速记资料范围内，内容供参考"。

【其他生成要求】
1. coreSymptoms 是4个汉字组成的数组（如["咳","痰","喘","肿"]），是该疾病最具特征性的4个字
2. differentialRows 4个鉴别诊断，每个包含 disease、symptom、key 三个字段
3. treatmentCards 4个治疗要点，每个包含 num、title、desc 三个字段
4. diagnosisPoints 3条诊断要点
5. westernTreatment 5条西医治疗原则（参考西医指南，分一般治疗、对症治疗、对因治疗、特殊治疗）
6. compliance 3条（医学准确性、专业术语规范、平台社区公约）
7. distribution.xiaohongshu 适合小红书发布的文案（带emoji和话题标签，300-500字）
8. distribution.wechat 适合公众号的长文格式（800-1200字，有小标题）
9. memoryCard.title 是疾病全称速记卡片
10. memoryCard.category 是科室·系统分类（如"内科·循环系统"）
11. memoryInfographic.topicBadge 是"有天同学·医考干货"
12. memoryInfographic.title 是"26中西医执医技能考点速记图"
13. memoryInfographic.subtitle 是疾病名（含中医病名/西医病名）
14. memoryInfographic.footer 是"有天同学的医考干货 ｜ 26中西医技能必背"
15. distribution.shareLink 是分享链接（设为"https://ythub.work/flow/" + 疾病拼音缩写）
16. memoryInfographic.diagnosisStandard 是该疾病的诊断标准概述（必须随疾病改变）
17. memoryInfographic.keyDiagnosisCriteria 是该疾病最具特异性、最必背的诊断标准（必须随疾病改变，是关键指标/阈值/体征）
18. 【附加要求——硬性满足，违反即为不合格】用户消息中可能含"【附加要求】xxx"段，是用户对本次生成的额外指令。硬性要求如下：
    (1) 附加要求中给出的具体数值、阈值、分类、分度、分期、条目（如"心率<60次/分""I度/II度/III度房室传导阻滞""病态窦房结综合征心率<50次/分"），必须逐条原样或近原样呈现到 diagnosisPoints / definition / keyDiagnosisCriteria 中，不得概括化、不得简化合并、不得遗漏任何一条。
    (2) 附加要求中若列出 N 个条目，diagnosisPoints 至少展开 N 条对应内容，每条对应一个亚型/分度，严禁压缩成 1-2 句概况。
    (3) 【禁止用"或"合并条目】严禁写成"窦性心动过缓、房室传导阻滞或病态窦房结综合征"这种把多个亚型塞进一句话的概况式表述。每个亚型必须是独立的一条 diagnosisPoints，且包含其特异性数值/分度标准。
    (4) 【禁止一句话覆盖】严禁用"符合...的特异性表现"这类模糊话术一笔带过，必须把每个亚型的具体诊断标准（数值、分度、心电图特征）写出来。
    (5) 【禁止自相矛盾】全文同一指标的前后数值必须完全一致，严禁出现"一处<60、另一处<50"的矛盾。若附加要求与知识库默认数值不同，以附加要求为准。
    (6) 在不违背知识库证型选方的前提下，必须满足附加要求的其他指令（如"重点讲房颤的鉴别""补充并发症处理"）。
19. 【多亚型疾病展开——硬性】凡诊断公式或附加要求中列出多个亚型/分度/分期的疾病，必须在 definition 和 diagnosisPoints 中逐条详细说明各亚型的特征性表现与鉴别要点，不得只给概况、不得合并条目。例如：
    - 快速性心律失常：窦速、房早、室早、阵发性室上速、房扑、房颤、室速，逐条列心电图特征。
    - 缓慢性心律失常：窦性心动过缓、房室传导阻滞(I度/II度I型/II度II型/III度)、病态窦房结综合征，逐条列诊断标准与数值。
    - 急性心肌梗死：ST段抬高型/非ST段抬高型，分别说明。
20. 【口诀格式——硬性规则，违反任意一条即为不合格】memoryCard.mnemonic 和 memoryInfographic.formulaMnemonic 必须是一首对仗工整、一韵到底、字数全篇严格对称、朗朗上口的口诀。硬性要求如下：
    (1)【字数严格对称·全篇一致】整首口诀只允许采用以下四种句式之一，且全篇每一小句都必须是该字数，严禁任何一句多一字或少一字：
        - 七字句：每小句恰好 7 字
        - 三三句：每小句为"3字+3字"
        - 四四句：每小句为"4字+4字"
        - 五五句：每小句为"5字+5字"
    (2)【长方名必须缩写】若某证型的方剂名（尤其含"合"字的复方，如"参附汤合桂枝甘草龙骨牡蛎汤""苓桂术甘汤合丹参饮"）超过目标句式容纳字数，必须缩写为该方的核心标识（取主方名或关键药味缩写）。仅在方名被缩写时，才在 mnemonicExplain 中用极简格式注明缩写对应关系（如"参附汤=参附汤合桂枝甘草龙骨牡蛎汤"），每条一行；未缩写的方剂不写、不解释。例如：
        - "参附汤合桂枝甘草龙骨牡蛎汤" → 七字句内可缩为"参附桂甘牡"或"参附桂枝汤"
        - "苓桂术甘汤合丹参饮" → 可缩为"苓桂丹参汤"
        - "柴胡疏肝散合胃苓汤" → 可缩为"柴疏合胃苓"或"柴疏胃苓汤"
        严禁因方名过长就破坏字数对称（如写成7字句+5字句）。
    (3)【一韵到底】全首口诀每一小句的末字必须押同一韵（允许近韵，不可乱韵、不可跳韵）。若方剂名末字不押该韵，须用谐音字替换或加垫字凑韵（如"生脉饮"→"生脉方"以与"保元汤"同押 ang 韵；"桃仁红花煎"→"桃红方"）。
    (4)【对仗对称】上下句结构相同（证型关键词 + 方名缩写），词性相对，节奏一致。
    (5)【一句一证型一方·全覆盖】每一小句对应一个证型 + 其代表方剂，必须覆盖该疾病在知识库中的全部证型，不可遗漏、不可合并。
    (6)【精简精准·谐音助记】仅用证型关键词 + 方名，不堆砌症状；鼓励用谐音、联想助记。
    (7)【生成后自检——必做】写完口诀后，必须逐句数字数，确认每一小句字数完全相同、末字同韵；若发现长短不一或跳韵，必须改写至完全合规后再输出。
    错误示例（禁止）：
      ✗ "心阳参附桂甘牡，心肾参附真武汤，气阴炙甘草，痰浊涤痰汤，心脉桃仁红花煎"（7/7/5/5/7，字数严重不一，且不押韵）
      ✗ "气虚血瘀保元汤，气阴两虚证用生脉饮"（前7字、后8字）
      ✗ "心虚胆怯安神定，心血不足归脾汤"（"定""汤"不押韵）
    正确示例1（慢性心衰·七字句·押 ang 韵·4证型全覆盖）：
      ✓ "气虚血瘀保元汤，气阴两虚生脉方，阳虚水泛真武汤，痰饮阻肺苓桂匡。"
    正确示例2（缓慢性心律失常·七字句·押 ang 韵·5证型全覆盖，长方名已缩写）：
      ✓ "心阳不振参附汤，心肾阳虚真武匡，气阴两虚炙草汤，痰浊涤痰桃红方，心脉瘀阻桃红汤。"
      （注："参附汤"代"参附汤合桂枝甘草龙骨牡蛎汤"；"真武匡"代"参附汤合真武汤"取真武+谐音；"炙草汤"代"炙甘草汤"；"桃红方/汤"代"桃仁红花煎"。末字汤/匡/汤/方/汤均押 ang 韵。）
    memoryCard.mnemonicExplain 和 memoryInfographic.formulaMnemonicExplain 仅在口诀中存在方名缩写时填写，格式为"缩写=全称"每条一行（如"参附汤=参附汤合桂枝甘草龙骨牡蛎汤\n真武匡=参附汤合真武汤"）；若口诀中方名均未缩写（全部使用原方名），则填空字符串 ""。严禁逐句解释证型、病机等冗余内容——口诀本身字数少、一眼可懂，解释只会浪费版面。

请直接返回JSON，不要有任何额外文字或markdown格式标记。

【极其重要】所有数组元素必须是"对象"或"字符串"按下方示例的结构，绝对不能把对象数组退化成字符串数组。具体要求：
- syndromes、formulaRows、differentialRows、treatmentCards、compliance 必须是「对象数组」，每个元素都是带固定字段的对象（绝不能只写证型名字符串）
- diagnosisPoints、westernTreatment、coreSymptoms 必须是「字符串数组」，每个元素都是字符串

JSON 顶层结构示例（必须严格按此字段结构填充）：
{
  "memoryCard": {
    "title": "肝硬化速记卡片",
    "category": "内科·消化系统",
    "definition": "...",
    "etiology": "...",
    "diagnosisPoints": ["要点1", "要点2", "要点3"],
    "syndromes": [
      {"type": "气滞湿阻证", "formula": "柴胡疏肝散合胃苓汤", "alt": false},
      {"type": "寒湿困脾证", "formula": "实脾饮", "alt": true}
    ],
    "westernTreatment": ["治疗1", "治疗2", "治疗3", "治疗4", "治疗5"],
    "mnemonic": "...",
    "mnemonicExplain": "..."
  },
  "memoryInfographic": {
    "topicBadge": "有天同学 · 医考干货",
    "title": "26中西医执医技能考点速记图",
    "subtitle": "肝硬化（鼓胀）",
    "coreSymptoms": ["鼓", "胀", "腹", "水"],
    "diagnosisStandard": "...",
    "keyDiagnosisCriteria": "...",
    "formulaRows": [
      {"type": "气滞湿阻证", "symptom": "腹胀按之不坚+胁胀痛", "formula": "柴胡疏肝散合胃苓汤", "alt": false},
      {"type": "寒湿困脾证", "symptom": "腹大胀满+得热则缓", "formula": "实脾饮", "alt": true}
    ],
    "formulaMnemonic": "...",
    "formulaMnemonicExplain": "...",
    "differentialRows": [
      {"disease": "肝硬化", "symptom": "...", "key": "...", "alt": false},
      {"disease": "结核性腹膜炎", "symptom": "...", "key": "...", "alt": true}
    ],
    "treatmentCards": [
      {"num": "1", "title": "一般治疗", "desc": "休息+营养支持"},
      {"num": "2", "title": "病因治疗", "desc": "..."}
    ],
    "footer": "有天同学的医考干货 ｜ 26中西医技能必背"
  },
  "compliance": [
    {"label": "医学准确性", "description": "..."},
    {"label": "专业术语规范", "description": "..."},
    {"label": "平台社区公约", "description": "..."}
  ],
  "distribution": {"xiaohongshu": "...", "wechat": "...", "shareLink": "https://ythub.work/flow/gzh"}
}

【考试资料知识库】
以下是2026年中西医执业医师实践技能考试第一站辨证选方速记官方资料，所有中医证型与选方必须严格以此为准：

${examKnowledge}
`

// ===== 后处理校验：LLM 不擅长数字数，必须外部强校验 + 自动重试 =====

// 拆分口诀小句：按中文标点（，；。）拆分，剔除空串
function splitMnemonicClauses(mnemonic: string): string[] {
  return mnemonic
    .split(/[，；。\n,;.]/)
    .map((s) => s.trim())
    .filter((s) => s.length > 0)
}

// 末字拼音末尾（极简映射，仅覆盖口诀常用字；未命中则跳过韵脚校验）
const CHAR_TAIL: Record<string, string> = {
  汤: 'ang', 方: 'ang', 匡: 'ang', 阳: 'ang', 旺: 'ang', 凉: 'ang', 香: 'ang', 羌: 'ang', 姜: 'ang', 邦: 'ang',
  丸: 'an', 丹: 'an', 散: 'an', 甘: 'an', 寒: 'an', 欢: 'an', 宽: 'an', 还: 'an', 参: 'an', 肝: 'an',
  饮: 'in', 金: 'in', 心: 'in', 林: 'in', 阴: 'in', 琴: 'in',
  桂: 'ei', 牡: 'u', 草: 'ao', 脾: 'i', 血: 'e', 瘀: 'ü', 痰: 'an',
  黄: 'ang', 苓: 'ing', 泻: 'e', 萎: 'ei', 薤: 'ai', 枝: 'i', 附: 'u', 龙: 'ong',
}

function charRhymeTail(ch: string): string | null {
  if (CHAR_TAIL[ch]) return CHAR_TAIL[ch]
  return null
}

interface MnemonicIssue {
  field: string
  reason: string
  detail: string
}

// 校验单条口诀：字数全篇一致 + 末字同韵
function checkMnemonic(field: string, mnemonic: string): MnemonicIssue[] {
  const issues: MnemonicIssue[] = []
  if (!mnemonic) {
    issues.push({ field, reason: 'empty', detail: '口诀为空' })
    return issues
  }
  const clauses = splitMnemonicClauses(mnemonic)
  if (clauses.length < 2) {
    issues.push({ field, reason: 'too_short', detail: `口诀仅 ${clauses.length} 句，应覆盖全部证型` })
    return issues
  }
  // 字数一致性：每小句字数必须完全相同
  const lenSet = new Set(clauses.map((c) => c.length))
  if (lenSet.size > 1) {
    issues.push({
      field,
      reason: 'uneven_length',
      detail: `各句字数不一致：${clauses.map((c, i) => `第${i + 1}句「${c}」=${c.length}字`).join('；')}，必须全部相同`,
    })
  }
  // 末字同韵
  const tails = clauses.map((c) => charRhymeTail(c[c.length - 1]))
  const validTails = tails.filter((t): t is string => t !== null)
  if (validTails.length === tails.length && validTails.length > 1) {
    const tailSet = new Set(validTails)
    if (tailSet.size > 1) {
      issues.push({
        field,
        reason: 'not_rhymed',
        detail: `末字不同韵：${clauses.map((c, i) => `「${c}」末字「${c[c.length - 1]}」=${tails[i]}`).join('；')}，必须同韵`,
      })
    }
  }
  return issues
}

// 校验附加要求里的条目是否在 diagnosisPoints 中逐条、独立出现
function checkSubtypes(extra: string, diagnosisPoints: string[]): MnemonicIssue[] {
  const issues: MnemonicIssue[] = []
  if (!extra) return issues
  // 全角/半角罗马数字归一化（Ⅱ ↔ II），便于匹配
  const norm = (s: string): string =>
    s.replace(/[ⅡⅢⅣⅥⅦⅧⅨⅩ]/g, (ch) => ({ 'Ⅱ': 'II', 'Ⅲ': 'III', 'Ⅳ': 'IV', 'Ⅵ': 'VI', 'Ⅶ': 'VII', 'Ⅷ': 'VIII', 'Ⅸ': 'IX', 'Ⅹ': 'X' }[ch] || ch))
     .replace(/[lⅰ]/g, 'I')
  const lines = extra.split('\n').map((l) => l.trim()).filter(Boolean)
  const subItems: string[] = []
  for (const line of lines) {
    // 顶层编号："1.窦性心动过缓" "2.房室传导阻滞" "3.病态窦房结综合征"
    let m = line.match(/^\d+[.、)]\s*(.+)$/)
    if (m) {
      const title = m[1].split(/[(:：（]/)[0].trim()
      if (title) subItems.push(norm(title))
      continue
    }
    // 子层级编号："(1) I度" "(2) Ⅱ度" "(3) Ⅲ度" "1)l型" "2)Ⅱ型"
    m = line.match(/^\(?\d+\)\s*(.+)$/)
    if (m) {
      const title = m[1].split(/[(:：（]/)[0].trim()
      if (title && title.length >= 2) subItems.push(norm(title))
    }
  }
  if (subItems.length === 0) return issues
  const normDp = diagnosisPoints.map((dp) => norm(dp.trim()))
  // 区分顶层条目（疾病名）和子层级条目（分度/分型），squeeze 检测只用顶层条目
  const topItems = subItems.filter((it) => !/^[IVXⅠⅡⅢⅣⅥⅦⅧⅨⅩ]+度$|^[IVXⅠⅡⅢⅣⅥⅦⅧⅨⅩ]+型$/.test(it))
  // 检查每个条目是否作为"独立一条 diagnosisPoints 的主题"出现
  const missing: string[] = []
  for (const item of subItems) {
    const asStandalone = normDp.some((s) => {
      return new RegExp(`^(?:\\d+[.、)]\\s*)?${escapeReg(item)}`).test(s) || s.includes(item)
    })
    if (!asStandalone) {
      missing.push(item)
    }
  }
  // 检测"或"串列模式：一条 diagnosisPoints 里用"或"连接多个诊断特征
  const orSeriesPattern = /[^，。；\n]{2,}，?或[^，。；\n]{2,}，?或[^，。；\n]{2,}/
  const mergedDp = diagnosisPoints.filter((dp) => orSeriesPattern.test(dp))
  // 检测多个"顶层亚型"挤在同一条 diagnosisPoints 里（如"窦缓...房室传导阻滞...病窦"在一条里）
  const squeezePattern = diagnosisPoints.filter((dp) => {
    const s = norm(dp)
    let hits = 0
    for (const item of topItems) if (s.includes(item)) hits++
    return hits >= 3
  })
  if (missing.length > 0) {
    issues.push({
      field: 'diagnosisPoints',
      reason: 'subtype_missing',
      detail: `附加要求列出的条目未在 diagnosisPoints 中逐条独立出现：${missing.join('、')}。必须每个亚型/分度独立成一条，含其特异性数值/分度标准，禁用"或"合并、禁用一句话覆盖。`,
    })
  }
  if (mergedDp.length > 0) {
    issues.push({
      field: 'diagnosisPoints',
      reason: 'merged_by_or',
      detail: `检测到用"或"串列多个诊断特征的概况式表述（如"${mergedDp[0].slice(0, 60)}..."），必须拆成独立条目。`,
    })
  }
  if (squeezePattern.length > 0) {
    issues.push({
      field: 'diagnosisPoints',
      reason: 'squeezed_in_one',
      detail: `检测到多个亚型被挤在同一条 diagnosisPoints 里（"${squeezePattern[0].slice(0, 60)}..."），必须每个亚型独立成条。`,
    })
  }
  return issues
}

function escapeReg(s: string): string {
  return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
}

// 校验整份生成结果
function validateResult(parsed: any, extra: string): MnemonicIssue[] {
  const issues: MnemonicIssue[] = []
  const mc = parsed?.memoryCard
  const mi = parsed?.memoryInfographic
  if (mc?.mnemonic) issues.push(...checkMnemonic('memoryCard.mnemonic', mc.mnemonic))
  if (mi?.formulaMnemonic) issues.push(...checkMnemonic('memoryInfographic.formulaMnemonic', mi.formulaMnemonic))
  const dp: string[] = Array.isArray(mc?.diagnosisPoints) ? mc.diagnosisPoints : []
  issues.push(...checkSubtypes(extra, dp))
  return issues
}

/**
 * 从模型输出里提取 JSON 字符串（兜底，DeepSeek 官网 JSON mode 下通常用不到）。
 */
function extractJson(content: string): string {
  const trimmed = content.trim()
  try { JSON.parse(trimmed); return trimmed } catch { /* 继续兜底 */ }
  const fenceMatch = trimmed.match(/```(?:json)?\s*([\s\S]*?)```/i)
  if (fenceMatch) {
    const inner = fenceMatch[1].trim()
    try { JSON.parse(inner); return inner } catch { /* 继续兜底 */ }
  }
  const first = trimmed.indexOf('{')
  const last = trimmed.lastIndexOf('}')
  if (first !== -1 && last > first) {
    return trimmed.slice(first, last + 1)
  }
  return trimmed
}

export function deepseekApiPlugin(apiKey: string): Plugin {
  // 本地开发默认用 DeepSeek 官网 API（与生产端一致）
  const apiBase = 'https://api.deepseek.com/v1'
  // DeepSeek 官网模型：旧版 deepseek-chat 已下线，现支持 deepseek-v4-pro / deepseek-v4-flash
  const modelName = process.env.API_MODEL || 'deepseek-v4-flash'
  return {
    name: 'deepseek-api-plugin',
    configureServer(server) {
      server.middlewares.use('/api/generate', async (req, res) => {
        if (req.method === 'OPTIONS') {
          res.setHeader('Access-Control-Allow-Origin', '*')
          res.setHeader('Access-Control-Allow-Headers', 'Content-Type')
          res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS')
          res.statusCode = 204
          res.end()
          return
        }

        if (req.method !== 'POST') {
          res.statusCode = 405
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ error: 'Method Not Allowed' }))
          return
        }

        res.setHeader('Access-Control-Allow-Origin', '*')
        res.setHeader('Content-Type', 'application/json')

        let body = ''
        req.on('data', (chunk) => { body += chunk })
        req.on('end', async () => {
          try {
            const { topic, extra } = JSON.parse(body)
            if (!topic?.trim()) {
              res.statusCode = 400
              res.end(JSON.stringify({ error: '缺少 topic 参数' }))
              return
            }

            // 用户消息：疾病名 + 附加要求（若有）
            const extraTrim = extra?.trim() || ''
            const userMessage = extraTrim
              ? `${topic}\n\n【附加要求】${extraTrim}`
              : topic

            // 调用 DeepSeek（带重试）：首轮用配置模型，校验失败则升级到 pro 并附错误反馈重试一次
            async function callDeepSeek(messages: Array<{ role: string; content: string }>, model: string): Promise<string> {
              const requestBody: Record<string, unknown> = {
                model,
                messages,
                temperature: 0.3,
                response_format: { type: 'json_object' },
              }
              const response = await fetch(`${apiBase}/chat/completions`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${apiKey}`,
                },
                body: JSON.stringify(requestBody),
              })
              if (!response.ok) {
                const errorText = await response.text()
                throw new Error(`DeepSeek API 错误: ${errorText}`)
              }
              const data = (await response.json()) as { choices?: Array<{ message?: { content?: string } }> }
              const content = data.choices?.[0]?.message?.content
              if (!content) throw new Error('API 返回内容为空')
              return content
            }

            let content: string
            try {
              content = await callDeepSeek(
                [
                  { role: 'system', content: SYSTEM_PROMPT },
                  { role: 'user', content: userMessage },
                ],
                modelName,
              )
            } catch (e) {
              const msg = e instanceof Error ? e.message : String(e)
              res.statusCode = 500
              res.end(JSON.stringify({ error: msg }))
              return
            }

            // JSON 解析
            let parsed: any
            try {
              parsed = JSON.parse(extractJson(content))
            } catch {
              res.statusCode = 500
              res.end(JSON.stringify({ error: '解析 JSON 失败', raw: content }))
              return
            }

            // 后处理校验：口诀字数一致 + 亚型逐条出现
            const issues = validateResult(parsed, extraTrim)
            if (issues.length > 0) {
              // 校验失败：升级到 pro 模型 + 错误反馈重试一次
              const feedback = issues.map((i) => `[${i.field}] ${i.reason}: ${i.detail}`).join('\n')
              const retrySystem = `${SYSTEM_PROMPT}\n\n【上一轮生成结果校验失败，必须修复以下问题后重新输出完整 JSON】\n${feedback}\n\n注意：\n- 口诀每小句字数必须完全相同，长方名必须缩写（如"参附汤合桂枝甘草龙骨牡蛎汤"→"参附桂枝汤"），方名末字不押韵时用谐音字（如"生脉饮"→"生脉方"）。\n- 附加要求里每个编号条目必须是 diagnosisPoints 中独立的一条，禁用"或"合并、禁用一句话覆盖。\n- 修复后只输出完整 JSON，不要任何额外文字。`
              try {
                const retryContent = await callDeepSeek(
                  [
                    { role: 'system', content: retrySystem },
                    { role: 'user', content: userMessage },
                  ],
                  'deepseek-v4-pro',
                )
                // 用重试结果（即使仍有小瑕疵，pro 通常更好）
                res.statusCode = 200
                res.end(retryContent)
                return
              } catch {
                // 重试失败则返回首轮结果（降级）
              }
            }

            res.statusCode = 200
            res.end(JSON.stringify(parsed))
          } catch (err) {
            res.statusCode = 500
            res.end(JSON.stringify({
              error: '服务器内部错误',
              message: err instanceof Error ? err.message : String(err),
            }))
          }
        })
      })
    },
  }
}
