...
// 知识库内联（由 1-中西医内科诊断公式+辨证秒杀词(OCR).pdf 用 pymupdf 提取）
...
// 与生产端 api/generate.ts 保持一致的 system prompt（全量注入）
...
20. 【口诀格式——硬性规则，违反任意一条即为不合格】memoryCard.mnemonic 和 memoryInfographic.formulaMnemonic 必须是一首对仗工整、一韵到底、字数全篇严格对称、朗朗上口的口诀。硬性要求如下：
    ...
    (5)【一句含两证型两方·精简全覆盖】每一小句尽量包含两个证型 + 两个方剂（上下半句各一），以最大化信息密度、精简口诀长度。覆盖该疾病在知识库中的全部证型，不可遗漏。若某证型知识库给出"A方或B方""A方合B汤"等多方，口诀中只取其一（取较短、较易押韵者），另一方不写，以减轻记忆负担。
    (6)【精简精准·谐音助记】仅用证型关键词 + 方名，不堆砌症状；鼓励用谐音、联想助记；能用一句说完就不用两句，全首口诀行数越少越好。
    ...
    正确示例1（慢性心衰·七字句·押 ang 韵·4证型全覆盖，每句两证型两方）：
      ✓ "气虚保元瘀血府，气阴生脉阳真武，痰饮苓桂术甘主。"（3句覆盖4证型：气虚血瘀用保元汤/血府逐瘀汤取保元；气阴两虚用生脉散；阳虚水泛用真武汤；痰饮阻肺用苓桂术甘汤）
    正确示例2（缓慢性心律失常·七字句·押 ang 韵·5证型全覆盖，长方名已缩写，每句含两证型两方）：
      ✓ "心阳参附肾真武，气阴炙草痰涤汤，心脉瘀阻桃红方。"（3句覆盖5证型：心阳不振参附汤；心肾阳虚真武汤；气阴两虚炙甘草汤；痰浊阻滞涤痰汤；心脉瘀阻桃仁红花煎）
    memoryCard.mnemonicExplain 和 memoryInfographic.formulaMnemonicExplain 仅当口诀采用"三三句"（每小句3字+3字）时才填写，格式为"缩写=全称"每条一行；其余句式（七字句/四四句/五五句）一律填空字符串 ""。理由：七字及以上口诀已足够通俗直白，无需解释；唯有三字句因高度缩写才需注明。
